...
description: 'Phase 8B: Executes test plan via playwright-cli, tracks bugs in bugs/<BUG-NNN>.md,
  agent auto-fixes bugs and retests, performs gap analysis when bugs require spec
  changes. Loop continues until all P0/P1 bugs closed and exit criteria met. Use with:
  "run tests", "execute tests", "/speckit.product-forge.test-run"'
